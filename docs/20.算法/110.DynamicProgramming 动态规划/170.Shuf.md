...
给定一个未知大小的数据集，按随机顺序获得一个随机样本。这通常被用于数据分析，作为一种从数据池或仓库或CSV文件中获取小的随机样本的方法。
...
- [Sampling from a population of unknown size | by Andrea C. | Towards Data Science](https://towardsdatascience.com/sampling-from-a-population-of-unknown-size-1b48fbfb3b33)